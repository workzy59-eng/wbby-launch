    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          >
            {/* Background Glow */}
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-1/2 h-1/2 bg-[#cfcb11]/5 blur-[100px] pointer-events-none" />
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <Calendar className="absolute left-4 top-1/2 -translate-y-1/2 text-[#cfcb11]" size={16} />
                    <input
                      type="date"
                      required
                      value={formData.preferredDate}
                      onChange={(e) => setFormData({ ...formData, preferredDate: e.target.value })}
                      className="w-full bg-white/5 border border-white/10 rounded-2xl py-3 pl-12 pr-4 text-white text-sm focus:outline-none focus:border-[#cfcb11]/50 transition-colors"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <Clock className="absolute left-4 top-1/2 -translate-y-1/2 text-[#cfcb11]" size={16} />
                    <input
                      type="time"
                      required
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <MessageSquare className="absolute left-4 top-4 text-[#cfcb11]" size={16} />
                  <textarea
                    value={formData.message}
                    onChange={(e) => setFormData({ ...formData, message: e.target.value })}
                    placeholder="What would you like to discuss?"
                    rows={4}
                    className="w-full bg-white/5 border border-white/10 rounded-2xl py-3 pl-12 pr-4 text-white text-sm focus:outline-none focus:border-[#cfcb11]/50 transition-colors resize-none"
                  />
                </div>
              </div>
              ...
              <button
                ...
                className="w-full bg-[#cfcb11] hover:bg-[#cfcb11]/80 disabled:opacity-50 text-black font-black uppercase tracking-widest py-4 rounded-2xl transition-all flex items-center justify-center gap-2 group"
              >
                {isSubmitting ? (
