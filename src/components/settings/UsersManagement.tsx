    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <div className="bg-white/5 border border-white/10 rounded-[2rem] p-6 space-y-2">
          ...
          <div className="flex items-end justify-between">
            ...
            <Users className="text-[#FACC15]/20" size={32} />
          </div>
        </div>
        <div className="bg-white/5 border border-white/10 rounded-[2rem] p-6 space-y-2">
        ...
        <div className="bg-white/5 border border-white/10 rounded-[2rem] p-6 space-y-2">
          ...
          <div className="flex items-end justify-between">
            <h4 className="text-3xl font-black text-[#FACC15] italic tracking-tighter">₹ {totalRevenue.toLocaleString()}</h4>
            <CreditCard className="text-[#FACC15]/20" size={32} />
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-4 justify-between items-center bg-white/5 border border-white/10 p-4 rounded-[2rem]">
        <div className="relative w-full lg:w-96 group">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-white/20 group-focus-within:text-[#FACC15] transition-colors" size={18} />
          <input 
            type="text"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Search by name, email or phone..."
            className="w-full bg-black/40 border border-white/5 rounded-2xl py-3 pl-12 pr-4 text-sm text-white outline-none focus:border-[#FACC15]/50 transition-all"
          />
        </div>
        ...
        <div className="flex flex-wrap items-center gap-4 w-full lg:w-auto">
          <select 
            ...
            className="flex-1 lg:flex-none bg-black/40 border border-white/5 rounded-2xl px-6 py-3 text-xs text-white outline-none focus:border-[#FACC15]/50 transition-all appearance-none"
          >
                ...
                <tr key={user.uid} className="group hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-8 py-6 text-right">
                    <button 
                      ...
                      className="p-3 bg-white/5 hover:bg-[#FACC15] hover:text-black rounded-2xl transition-all group-hover:scale-110"
                    >
                      <Eye size={18} />
                    </button>
                  </td>
              ...
              <div className="p-8 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
                ...
                <div className="flex items-center gap-3">
                  <button 
                    ...
                    className={`p-4 rounded-2xl transition-all ${isEditing ? 'bg-[#FACC15] text-black' : 'bg-white/5 text-white/40 hover:text-white'}`}
                  >
                    <Edit2 size={20} />
                  </button>
                  <button 
                  ...
                  <div className="space-y-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase tracking-widest text-white/40 ml-4">Full Name</label>
                          <input 
                            ...
                            className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 px-6 text-white text-sm outline-none focus:border-[#FACC15]/50 transition-all"
                          />
                      </div>
                      <div className="space-y-2">
                      ...
                      <div className="space-y-2">
                        ...
                        <select 
                          ...
                          className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 px-6 text-white text-sm outline-none focus:border-[#FACC15]/50 transition-all appearance-none"
                        >
                          <option value="Basic">Basic</option>
                          <option value="Standard">Standard</option>
                          <option value="Premium">Premium</option>
                    ...
                    <div className="flex justify-end gap-4">
                      ...
                      <button 
                        ...
                        className="px-8 py-4 bg-[#FACC15] text-black rounded-2xl text-[10px] font-black uppercase tracking-widest hover:scale-105 transition-all shadow-[0_0_20px_rgba(250,204,21,0.2)]"
                      >
                        Save Changes
                      </button>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="flex items-center justify-between p-4 bg-[#FACC15]/5 rounded-2xl border border-[#FACC15]/10">
                          <div className="flex items-center gap-4">
                            <CreditCard className="text-[#FACC15]" size={18} />
                            <div>
                              <p className="text-[8px] font-black uppercase tracking-widest text-[#FACC15]/60">Current Plan</p>
                              <p className="text-sm font-black text-white uppercase">{selectedUser.plan || 'Standard'}</p>
                            </div>
                          </div>
                          <span className="px-3 py-1 bg-[#FACC15] text-black rounded-full text-[8px] font-black uppercase tracking-widest">Active</span>
                        </div>
                      </div>
                    </div>
                        ...
                        <div className="flex items-center gap-4">
                          <button 
                            ...
                            className={`px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${
                              selectedUser.status === 'inactive' ? 'bg-green-500/10 text-green-500 hover:bg-green-500 hover:text-white' : 'bg-[#FACC15]/10 text-[#FACC15] hover:bg-[#FACC15] hover:text-black'
                            }`}
                          >
