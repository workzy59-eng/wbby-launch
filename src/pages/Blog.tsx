...
export default function Blog() {
    ...
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center space-y-6">
        <motion.div
          ...
          className="w-12 h-12 border-4 border-[#E6FF00] border-t-transparent rounded-full"
        />
        <p className="text-xs font-black uppercase tracking-[0.3em] text-[#E6FF00] animate-pulse italic">Loading Insights...</p>
      </div>
    );
  }
  ...
  return (
    <motion.div 
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="min-h-screen bg-black text-white selection:bg-[#E6FF00] selection:text-black"
    >
      <SEO 
      ...
      <section className="relative pt-40 pb-20 overflow-hidden px-6">
        <div className="absolute inset-0 bg-[#E6FF00]/5 blur-[120px] rounded-full -translate-y-1/2 translate-x-1/2" />
        <div className="relative z-10 text-center space-y-8 max-w-4xl mx-auto">
          <motion.div 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className="inline-flex items-center gap-2 px-4 py-1.5 bg-[#E6FF00]/10 border border-[#E6FF00]/20 rounded-full text-[10px] font-black uppercase tracking-[0.3em] text-[#E6FF00] italic"
          >
          ...
          >
            <div className="relative group">
              <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-white/20 group-focus-within:text-[#E6FF00] transition-colors" size={20} />
              <input 
                type="text"
                placeholder="Search articles..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full bg-white/5 border border-white/10 rounded-full py-6 pl-16 pr-8 text-white placeholder:text-white/20 focus:outline-none focus:border-[#E6FF00]/50 focus:bg-white/10 transition-all text-sm font-black uppercase tracking-widest"
              />
            </div>
          </motion.div>
        </div>
        ...
        <section className="px-6 pb-20">
          <div className="max-w-7xl mx-auto">
            <motion.div 
              ...
              className="group relative bg-white/5 border border-white/10 rounded-[4rem] overflow-hidden cursor-pointer hover:border-[#E6FF00]/30 transition-all"
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2">
                ...
                <div className="p-12 lg:p-24 flex flex-col justify-center space-y-10">
                  <div className="flex items-center gap-4">
                    <span className="px-5 py-2 bg-[#E6FF00] text-black text-[10px] font-black uppercase tracking-widest rounded-full shadow-[0_0_20px_rgba(230,255,0,0.3)]">Featured Story</span>
                    <span className="text-white/40 text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
                      <TrendingUp size={12} className="text-[#E6FF00]" /> {featuredPost.category}
                    </span>
                  </div>
                  <h2 className="text-5xl md:text-7xl font-black tracking-tighter uppercase italic leading-[0.9] text-white group-hover:text-[#E6FF00] transition-colors">
                    {featuredPost.title}
                  </h2>
                  ...
                  <div className="flex items-center justify-between pt-6 border-t border-white/5">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 bg-white/10 rounded-full flex items-center justify-center text-[#E6FF00] font-black text-xl">
                    ...
                    <div className="flex items-center gap-3 text-[#E6FF00] font-black text-xs uppercase tracking-[0.2em] group-hover:translate-x-2 transition-transform italic">
      ...
      <section className="px-6 sticky top-24 z-30 py-6 bg-black/80 backdrop-blur-xl border-y border-white/5">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-8 overflow-x-auto no-scrollbar">
              ...
              <button
                ...
                className={`px-8 py-3 rounded-full text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap border ${
                  selectedCategory === cat 
                    ? 'bg-[#E6FF00] text-black border-transparent shadow-[0_0_30px_rgba(230,255,0,0.2)]' 
          ...
          <div className="hidden md:flex items-center gap-2 text-white/20 text-[10px] font-black uppercase tracking-widest italic">
            <TrendingUp size={14} className="text-[#E6FF00]" /> {filteredPosts.length} Articles Found
          </div>
        </div>
      </section>
                ...
                >
                  <div className="relative h-72 overflow-hidden">
                    ...
                    <div className="absolute top-8 left-8">
                      <span className="px-5 py-2 bg-black/60 backdrop-blur-md border border-white/10 text-[#E6FF00] text-[10px] font-black uppercase tracking-widest rounded-full">
                        {post.category}
                      </span>
                    </div>
                  </div>
                  <div className="p-12 flex flex-col flex-1 space-y-8">
                    <div className="flex items-center gap-6 text-white/20 text-[10px] font-black uppercase tracking-widest">
                      <div className="flex items-center gap-2"><Calendar size={14} className="text-[#E6FF00]" /> {typeof post.date === 'string' ? post.date : post.date.toDate().toLocaleDateString()}</div>
                      <div className="flex items-center gap-2"><Clock size={14} /> 5 min read</div>
                    </div>
                    <h3 className="text-3xl font-black tracking-tighter uppercase italic leading-tight text-white group-hover:text-[#E6FF00] transition-colors line-clamp-2">
                      {post.title}
                    </h3>
                    ...
                    <div className="pt-8 flex items-center justify-between border-t border-white/5">
                      <div className="flex items-center gap-3 text-white/40 text-[10px] font-black uppercase tracking-widest">
                        <div className="w-8 h-8 bg-white/10 rounded-full flex items-center justify-center text-[#E6FF00] font-black">
                          {post.author[0]}
                        </div>
                        {post.author}
                      </div>
                      <div className="flex items-center gap-2 text-[#E6FF00] font-black text-[10px] uppercase tracking-widest group-hover:translate-x-2 transition-transform italic">
          ...
          {filteredPosts.length === 0 && (
            <div className="py-40 text-center space-y-8">
              ...
              <button 
                ...
                className="px-12 py-5 bg-[#E6FF00] text-black rounded-full font-black text-xs uppercase tracking-widest hover:scale-105 transition-all shadow-xl"
              >
                Clear All Filters
              </button>
            </div>
