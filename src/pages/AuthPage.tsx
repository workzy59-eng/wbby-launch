...
import { AlertCircle, ArrowRight, Mail, Lock, CheckCircle2, ShieldCheck, Key } from 'lucide-react';
import { useAuth } from '../context/AuthContext';
import { toast } from 'react-hot-toast';
import { verifyDeveloperInvite, useDeveloperInvite, createUserProfile } from '../services/database';

export default function AuthPage() {
  const navigate = useNavigate();
  const location = useLocation();
  const { signInWithGoogle, user, signInWithEmail, signUpWithEmail } = useAuth();
  
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [isSignUp, setIsSignUp] = useState(false);
  const [isDevJoin, setIsDevJoin] = useState(false);
  ...
  const handleVerifyDevCode = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email || !inviteCode) {
    ...
    setError(null);
    try {
      const invite = await verifyDeveloperInvite(email, inviteCode);
      if (invite) {
        setVerifiedInvite(invite);
        setIsSignUp(true);
        toast.success('Clearance verified! Set your access logic.');
      } else {
        toast.error('Invalid or expired clearance code');
      }
    } catch (err: any) {
      setError(err.message);
    } finally {
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        ...
        if (newUser) {
          const additionalData = verifiedInvite ? {
            role: (verifiedInvite.role || 'developer') as any,
            displayName: verifiedInvite.name,
            permissions: verifiedInvite.permissions
          } : {};
          
          await createUserProfile(newUser, additionalData);
          
          if (verifiedInvite) {
            await useDeveloperInvite(verifiedInvite.id);
          }
        }
        toast.success(verifiedInvite ? 'Developer clearance granted!' : 'Account created successfully!');
      } else {
  ...
  return (
    <div className="min-h-screen bg-[#050505] flex items-center justify-center p-6 selection:bg-[#c7c42a] selection:text-black technical-grid">
      ...
      >
        ...
        {isDevJoin && !verifiedInvite ? (
          <form onSubmit={handleVerifyDevCode} className="space-y-6 mb-12">
            <div className="space-y-2">
              <label className="text-[10px] font-black text-[#c7c42a] uppercase tracking-[0.4em] ml-1 italic">Developer Email</label>
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-[#c7c42a] uppercase tracking-[0.4em] ml-1 italic">5-Digit Clearance Code</label>
              <div className="relative">
                ...
                <input 
                  ...
                  placeholder="00000"
                  className="w-full bg-[#050505] border border-white/5 rounded-none py-6 pl-14 pr-6 text-white focus:border-[#c7c42a] focus:ring-0 outline-none transition-all font-mono text-2xl tracking-[0.5em] text-center"
                  required
                />
              </div>
            </div>
            ...
            <button 
              type="button"
              onClick={() => setIsDevJoin(false)}
              className="w-full text-[10px] font-black uppercase tracking-widest text-white/20 hover:text-white transition-colors py-2"
            >
              Cancel & Back
            </button>
          </form>
          ...
          <form onSubmit={handleEmailAuth} className="space-y-6 mb-12">
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full bg-[#050505] border border-white/5 rounded-none py-6 pl-14 pr-6 text-white focus:border-[#c7c42a] focus:ring-0 outline-none transition-all font-mono text-sm disabled:opacity-50"
                  required
                />
              </div>
            </div>
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-white/20 uppercase tracking-[0.4em] ml-1 italic">{isSignUp ? 'Set Access Logic' : 'Access Logic'}</label>
              <div className="relative">
            ...
            >
              {loading ? 'Processing...' : (
                <>
                  <span>{isSignUp ? (verifiedInvite ? 'Complete Clearance' : 'Configure') : 'Authorize'}</span>
                  <ArrowRight size={20} className="group-hover:translate-x-2 transition-transform" />
                </>
              )}
            </button>
        ...
        <div className="mt-12 text-center space-y-4">
          {!isDevJoin && !verifiedInvite && (
            <button 
              ...
              className="block w-full text-[10px] font-black uppercase tracking-widest text-white/30 hover:text-[#c7c42a] transition-colors italic"
            >
              {isSignUp ? 'Switch to Authorized Entry' : "Register New Identity Protocol"}
            </button>
          )}
          
           {!isDevJoin && !verifiedInvite && !isSignUp && (
            <button 
              onClick={() => setIsDevJoin(true)}
              className="block w-full text-[10px] font-black uppercase tracking-widest text-[#c7c42a] hover:opacity-80 transition-all italic underline decoration-dotted underline-offset-4"
            >
              Joining as Developer? Enter Clearance Code
            </button>
          )}
        </div>
