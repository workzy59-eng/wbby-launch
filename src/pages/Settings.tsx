        ...
        <div className="space-y-8">
          <div className="bg-white/5 border border-white/10 rounded-[2.5rem] p-8 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <button className="flex items-center justify-between p-6 bg-black/40 border-2 border-[#FACC15] rounded-3xl group">
                <div className="flex items-center gap-4">
                  <Moon className="text-[#FACC15]" size={20} />
                  <span className="text-sm font-black uppercase tracking-widest text-white">Dark Mode</span>
                </div>
                <Check className="text-[#FACC15]" size={20} />
              </button>
      ...
      <div className="max-w-7xl mx-auto space-y-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
          <div className="space-y-2">
            <button 
              onClick={() => navigate(-1)}
              className="flex items-center gap-2 text-white/40 hover:text-[#FACC15] transition-colors text-[10px] font-black uppercase tracking-widest mb-4"
            >
              <ArrowLeft size={14} />
              Back to Dashboard
            </button>
            <h1 className="text-5xl font-black uppercase italic tracking-tighter leading-none">
              Account <span className="text-[#FACC15]">Settings.</span>
            </h1>
            ...
            {tabs.map((tab) => (
              <button
                ...
                className={`w-full flex items-center justify-between p-6 rounded-[2rem] transition-all group ${
                  activeTab === tab.id 
                    ? 'bg-[#FACC15] text-black shadow-[0_0_30px_rgba(250,204,21,0.1)]' 
                    : 'bg-white/5 text-white/40 hover:bg-white/10 hover:text-white border border-white/5'
                }`}
              >
                <div className="flex items-center gap-4">
                  <tab.icon size={20} className={activeTab === tab.id ? 'text-black' : 'group-hover:text-[#FACC15] transition-colors'} />
            ...
            >
              <div className="mb-10 space-y-1">
                <h2 className="text-3xl font-black uppercase italic tracking-tighter text-white">
                  {tabs.find(t => t.id === activeTab)?.label} <span className="text-[#FACC15]">Details.</span>
                </h2>
