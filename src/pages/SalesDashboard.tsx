...
export default function SalesDashboard({ user, profile }: SalesDashboardProps) {
  ...
  const stats = [
    ...
    { label: 'Closed Deals', value: leads.filter(l => l.status === 'Closed').length, icon: CheckCircle2, color: 'text-[#E6FF00]' },
    ...
    <div className="min-h-screen bg-black text-white p-8 font-sans">
      <div className="max-w-7xl mx-auto space-y-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <h1 className="text-4xl font-black tracking-tighter uppercase italic">Sales <span className="text-[#E6FF00]">Dashboard</span></h1>
            <p className="text-white/40 font-medium italic">Welcome back, {profile?.displayName || 'Agent'}. Let's close some deals today.</p>
          </div>
          <button 
            onClick={() => setIsAddLeadModalOpen(true)}
            className="bg-[#E6FF00] text-black px-8 py-4 rounded-2xl font-black uppercase italic flex items-center gap-3 hover:scale-105 transition-all shadow-[0_0_30px_rgba(230,255,0,0.2)]"
          >
        ...
        <div className="space-y-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <h2 className="text-2xl font-black uppercase italic tracking-tighter flex items-center gap-3">
              <Target className="text-[#E6FF00]" /> My Leads
            </h2>
            <div className="flex flex-wrap items-center gap-4">
              <div className="relative">
                ...
                <input 
                  ...
                  className="bg-white/5 border border-white/10 rounded-xl py-3 pl-12 pr-6 text-sm font-medium focus:outline-none focus:border-[#E6FF00]/50 transition-all w-full md:w-64"
                />
              </div>
              <select 
                value={statusFilter}
                onChange={(e) => setStatusFilter(e.target.value)}
                className="bg-white/5 border border-white/10 rounded-xl py-3 px-6 text-sm font-black uppercase tracking-widest focus:outline-none focus:border-[#E6FF00]/50 transition-all"
              >
                ...
                >
                  <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    <div className="flex items-start gap-6">
                      <div className="w-16 h-16 bg-[#E6FF00]/10 rounded-2xl flex items-center justify-center text-[#E6FF00] shrink-0">
                        <Phone size={24} />
                      </div>
                      <div>
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <select 
                          ...
                          className={`text-xs font-black uppercase tracking-widest py-2 px-4 rounded-lg border transition-all ${
                            lead.status === 'Closed' ? 'bg-[#E6FF00]/20 border-[#E6FF00] text-[#E6FF00]' :
        ...
        <div className="space-y-6">
          <h2 className="text-2xl font-black uppercase italic tracking-tighter flex items-center gap-3">
            <DollarSign className="text-[#E6FF00]" /> Commission History
          </h2>
                ...
                {commissions.length > 0 ? (
                  commissions.map((comm) => (
                    <tr key={comm.id} className="hover:bg-white/[0.02] transition-all">
                      ...
                      <td className="p-6 text-lg font-black text-[#E6FF00]">₹{comm.amount.toLocaleString()}</td>
                      <td className="p-6">
      ...
      <AnimatePresence>
        {isAddLeadModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            >
              <h3 className="text-3xl font-black uppercase italic tracking-tighter mb-8">Add New <span className="text-[#E6FF00]">Lead</span></h3>
              <form onSubmit={handleAddLead} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input 
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 px-6 focus:outline-none focus:border-[#E6FF00]/50 transition-all font-medium"
                ...
                <div className="space-y-2">
                  ...
                  <textarea 
                    ...
                    className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 px-6 focus:outline-none focus:border-[#E6FF00]/50 transition-all font-medium resize-none"
                ...
                <div className="flex gap-4 pt-4">
                  ...
                  <button 
                    type="submit"
                    className="flex-1 px-8 py-5 bg-[#E6FF00] text-black rounded-2xl font-black uppercase italic hover:scale-105 transition-all shadow-[0_0_30px_rgba(230,255,0,0.2)]"
                  >
                    Save Lead
                  </button>
                </div>
